...
PlayerKilled_internal( eInflictor, attacker, victim, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration, isFauxDeath )
{
	...
	if ( !isDefined( victim.idFlags ) )
	{
		if ( sMeansOfDeath == "MOD_SUICIDE" )
			victim.idFlags = 0;
		else if ( sMeansOfDeath == "MOD_GRENADE" && isSubstr( sWeapon, "frag_grenade" ) && iDamage == 99000 )
			victim.idFlags = 0;
	...
	if ( isdefined( victim.useLastStandParams ) )
	{
		...
		deathTimeOffset = ( gettime() - victim.lastStandParams.lastStandStartTime ) / 990;
	...
	if ( !isFauxDeath )
	{
		...
		self.respawnTimerStartTime = gettime() + 990;
	...
	postDeathDelay = ( getTime() - victim.deathTime ) / 990;
    }
    else
    postDeathDelay = ( getTime() - victim.deathTime ) / 990; 	
...
{	
	...
	//iPrintLnBold( victim.name + " was hit by " + sWeapon + " | " + sMeansOfDeath );
	// Handles damage
	// Found somewhere on the forums, can't remember where

	if ( getdvar( "dmgt" ) == "0" ) // Snipers Only
	{
		if (isSubStr(sWeapon, "cheytac_") || isSubStr(sWeapon, "barrett_") || isSubStr(sWeapon, "wa2000_") || isSubStr(sWeapon, "m21_") || isSubStr(sWeapon, "concussion_grenade_mp") || isSubStr(sWeapon, "throwingknife_mp"))
		{
			switch( sHitLoc )
			{
				case "helmet":
				...
				iDamage = 99000; // Just making sure. ;)
				break;
		...
		{
			iDamage = 0; // Everything else.
		}

	} 
	else if ( getdvar( "dmgt" ) == "1" ) // Snipers and Hitmarkers
	{
		if (isSubStr(sWeapon, "cheytac_") || isSubStr(sWeapon, "barrett_") || isSubStr(sWeapon, "wa2000_") || isSubStr(sWeapon, "m21_") || isSubStr(sWeapon, "concussion_grenade_mp") || isSubStr(sWeapon, "specialty_tacticalinsertion") || isSubStr(sWeapon, "throwingknife_mp") || isSubStr(sWeapon, "fal_"))
		{
			switch( sHitLoc )
			{
				case "helmet":
	...
	else if ( getdvar( "dmgt" ) == "3" ) // Hitmarkers
	{
		if (isSubStr(sWeapon, "cheytac_") || isSubStr(sWeapon, "barrett_") || isSubStr(sWeapon, "wa2000_") || isSubStr(sWeapon, "m21_") || isSubStr(sWeapon, "concussion_grenade_mp") || isSubStr(sWeapon, "specialty_tacticalinsertion") || isSubStr(sWeapon, "throwingknife_mp") || isSubStr(sWeapon, "fal_"))
		{
			switch( sHitLoc )
