...
//			return false;
		...
		else if ( gameHasStarted() )
		{
			...
			if ( !level.inGracePeriod && !self.hasSpawned )
				return true; // Changing 'false' to 'true' so people can join laters :P
		}
	}
	return true;
}
...
//	if ( maps\mp\gametypes\_tweakables::getTweakableValue( "player", "forcerespawn" ) == 0 && self.hasSpawned && !waveBased && !self.wantSafeSpawn )
	...
	if ( getDvar( "g_gametype" ) == "sd" && self.pers["team"] == "allies" && getDvar( "testClients_doRespawn" ) == "0" && self.hasSpawned == true) {
